...
import {
  ...
  useSigner,
  useStargateClient,
} from 'packages/cosmos-connect/src'
import type { StringIndexed } from '@sifchain/ui'
import { caseInsensitiveRecord, memoizeWith } from 'packages/utils/src'
...
export function useAllBalancesQuery() {
  ...
  const indices = useMemo(() => {
    const indexedByDenom = caseInsensitiveRecord(
...
export function useBalancesWithPool() {
  const { findBySymbolOrDenom: findTokenBySymbolOrDenom } =
    useTokenRegistryQuery()
  const { data: liquidityProviders } = useLiquidityProvidersQuery()
  const { data: balances, findBySymbolOrDenom: findBalanceBySymbolOrDenom } =
    useAllBalancesQuery()
  const { data: env } = useDexEnvironment()
