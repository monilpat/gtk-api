...
>(optimisticHistory: { results: T['results'] }, positions: J) {
  ...
  const optimisticClosedMatch = optimisticHistory.results.filter(x =>
    newDataIds.includes(x.id)
  )
  ...
  if (optimisticClosedMatch.length) {
    ...
    draft.pagination.limit = String(
      Number(positions.pagination.limit) - optimisticClosedMatch.length
    )
    return draft
  }
