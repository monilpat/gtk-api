...
export function useMarginOpenPositionsBySymbolQuery({
  poolSymbol,
}: {
  poolSymbol: string
}) {
  ...
  return useSifApiQuery(
    ...
    ],
    {
      ...
      structuralSharing(
        ...
        if (newData) {
          const diffOptimisticPositions = (queryClient.getQueryData([
            'margin.getOptimisticPositions',
          ]) ?? []) as MarginOpenPositionsResponse['results']

          const diffOptimisticHistory = (queryClient.getQueryData([
