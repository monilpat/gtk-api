...
export function useMarginMTPOpenMutation(props: UseMarginMTPOpenMutationProps) {
  ...
  async function mutation(variables: OpenMTPVariables) {
    ...
    if (res && isDeliverTxFailure(res) && res.rawLog) {
      ...
      // ========================================
      if (
        res.rawLog.includes(errors.SIFNODE_ERRORS.MARGIN_NOT_ENABLED_FOR_POOL)
      ) {
        throw new Error(errors.POOL_TRADE_TEMPORARILY_DISABLED)
      }

      if (
        res.rawLog.includes(errors.SIFNODE_ERRORS.MAX_OPEN_POSITIONS_REACHED)
      ) {
      ...
      // ========================================
      if (
        res.rawLog.includes(errors.SIFNODE_ERRORS.BORROWED_AMOUNT_IS_TOO_LOW)
      ) {
      ...
      if (
        res.rawLog.includes(
          errors.SIFNODE_ERRORS.BORROWED_AMOUNT_IS_HIGHER_THAN_POOL_DEPTH
        )
      ) {
