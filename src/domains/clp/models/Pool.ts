...
export default class Pool {
  ...
  calculateSwap(params: { inputAmount: string; inputDenom: string }) {
    return params.inputDenom === ROWAN.symbol
      ? this.calculateSwapFromRowan(params)
      : this.calculateSwapToRowan(params)
  }

  calculateMarginPosition(params: {
    inputAmount: string
    inputDenom: string
    leverage: number
  }) {
    return this.calculateSwap({
      inputAmount: BigNumber(params.inputAmount)
