import type { IAsset } from 'packages/common/src'
import type { GetTokenStatsResponsePools } from 'packages/sif-api/src'
import {
  ascend,
...
export function useEnhancedPoolsQuery() {
  ...
  const indices = useMemo(() => {
    ...
    const indexedBySymbol = caseInsensitiveRecord(
      indexBy(x => x.asset.symbol, derivedQuery.data)
    )
