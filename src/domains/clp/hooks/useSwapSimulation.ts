...
import { runCatching } from 'packages/common/src'
import type { SifSigningStargateClient } from '@sifchain/stargate'
import { Maybe } from 'packages/utils'
...
type SimulateSwapSyncArgs = ArgumentTypes<
  SifSigningStargateClient['simulateSwapSync']
>
...
export function useSwapSimulationQuery(
  ...
  const { data: swapFeeRateResult } = useSifnodeQuery(
    'clp.getSwapFeeParams',
    [{}],
    COMMON_OPTIONS
  )
  const { data: marginParamsResult } = useSifnodeQuery(
    'margin.getParams',
    [{}],
    COMMON_OPTIONS
  )
  ...
  const poolSwapFeeRate = swapFeeRateResult?.tokenParams.find(
    propEq('asset', fromDenom)
  )

  const swapFeeRate = Maybe.of(
    poolSwapFeeRate?.swapFeeRate ?? swapFeeRateResult?.defaultSwapFeeRate
  ).mapOr('0', x => Decimal.fromAtomics(x, 18).toString())
  ...
  const compute = useCallback(
    (amount = fromAmount) => {
      ...
      const [_, fromAmountDecimal] = runCatching(() =>
        Decimal.fromUserInput(amount, fromToken?.decimals ?? 0)
      )
      ...
      const [__, result] = runCatching(() => {
        const fromCoin: SimulateSwapSyncArgs[0] = {
          ...
          isMarginEnabled: marginEnabledPools.has(
            fromPool?.externalAsset?.symbol ?? ''
          ),
        }
...
const withLeverage = (amount: string, leverage: string | number) =>
  BigNumber(amount).times(leverage).toString()

export function useMarginPositionSimulationQuery(
  fromDenom: string,
  toDenom: string,
  fromAmount: string,
  leverage = 0
) {
  const leveragedAmount = useMemo(
    () => withLeverage(fromAmount, leverage),
    [fromAmount, leverage]
  )

  const derivedQuery = useSwapSimulationQuery(
    fromDenom,
    toDenom,
    leveragedAmount,
    0
